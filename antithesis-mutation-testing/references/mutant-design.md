...
- **Non-fatal.** The mutant must not crash a container deterministically, hang startup, or prevent `setup_complete`. A mutant that cannot get the system up produces an off-target failure instead of a falsification. This is about *deterministic* crashes the mutant causes: Antithesis kills and stops containers as injected faults and restarts them afterwards, so a container exiting during a run is routine and is not a signal about the mutant.
