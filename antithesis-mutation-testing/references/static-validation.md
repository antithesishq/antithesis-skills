...
**It may not authorize a change to the user's source on its own.** A trace that
says the oracle is too loose is a hypothesis with a cheap test: launch the
mutant, record the prediction, and fix the oracle once the run agrees.

**Where a fix is genuinely required before launching** — most often a workload
that cannot reach the mutated code at all — it is still a real-tree change:
honor interview question 4, batch it with the round's other real-tree fixes,
and re-fork plus re-baseline before the sweep.
