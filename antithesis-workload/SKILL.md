...
- No reach claim is the logical negation of an `Always` or `AlwaysOrUnreachable` at the same site, and every reach claim still fires on a build with every bug fixed (see `references/assertions.md`, "A Reach Claim Asserts The Precondition, Never The Violation")
