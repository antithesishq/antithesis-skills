...
`snouty` must be at least version 0.5.0. Then confirm credentials and API
access with the canonical check (same as the triage skill's preflight):
...
`$ANTITHESIS_TENANT` is empty in the current shell (it may be configured
elsewhere, e.g. a `.env`, so this is not a failure), ask the user for the tenant
