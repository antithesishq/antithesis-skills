...
ALWAYS USE `--session-name antithesis` when creating a new session. This ensures that authentication persists across multiple sessions and minimizes how often the user will need to re-authenticate.
...
Before continuing make sure that the `antithesis` session-name is already authenticated. You can check this via:
...
If you need to authenticate, read `references/setup-auth.md`.
