...
- If `ANTITHESIS_REPOSITORY` is not available, ask the user for the registry value before writing compose image tags, and tell them to export it before running `antithesis/submit.sh`.
- Build compose services referenced via `build:` in `submit.sh` before invoking `snouty run`
